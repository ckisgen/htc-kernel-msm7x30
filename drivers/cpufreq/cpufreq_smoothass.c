...
enum {
        SMOOTHASS_DEBUG_JUMPS=1,
        SMOOTHASS_DEBUG_LOAD=2
};
...
static void smoothass_update_min_max(struct smoothass_info_s *this_smoothass, struct cpufreq_policy *policy, int suspend) {
        if (suspend) {
                this_smoothass->min_speed = policy->min;
                this_smoothass->max_speed = // sleep_max_freq; but make sure it obeys the policy min/max
                        policy->max > sleep_max_freq ? (sleep_max_freq > policy->min ? sleep_max_freq : policy->min) : policy->max;
        } else {
                this_smoothass->min_speed = // awake_min_freq; but make sure it obeys the policy min/max
                        policy->min < awake_min_freq ? (awake_min_freq < policy->max ? awake_min_freq : policy->max) : policy->min;
                this_smoothass->max_speed = policy->max;
        }
}

inline static unsigned int validate_freq(struct smoothass_info_s *this_smoothass, int freq) {
        if (freq > this_smoothass->max_speed)
                return this_smoothass->max_speed;
        if (freq < this_smoothass->min_speed)
                return this_smoothass->min_speed;
        return freq;
}

static void reset_timer(unsigned long cpu, struct smoothass_info_s *this_smoothass) {
  this_smoothass->time_in_idle = get_cpu_idle_time_us(cpu, &this_smoothass->idle_exit_time);
...
static void cpufreq_smoothass_timer(unsigned long data)
{
        ...
        if (this_smoothass->idle_exit_time == 0 || update_time == this_smoothass->idle_exit_time)
                return;

        delta_idle = cputime64_sub(now_idle, this_smoothass->time_in_idle);
        delta_time = cputime64_sub(update_time, this_smoothass->idle_exit_time);
        //printk(KERN_INFO "smoothassT: t=%llu i=%llu\n",cputime64_sub(update_time,this_smoothass->idle_exit_time),delta_idle);

        // If timer ran less than 1ms after short-term sample started, retry.
        if (delta_time < 1000) {
                if (!timer_pending(&this_smoothass->timer))
                        reset_timer(data,this_smoothass);
                return;
        }
        ...
        if (debug_mask & SMOOTHASS_DEBUG_LOAD)
                printk(KERN_INFO "smoothassT @ %d: load %d (delta_time %llu)\n",policy->cur,cpu_load,delta_time);

        this_smoothass->cur_cpu_load = cpu_load;

        // Scale up if load is above max or if there where no idle cycles since coming out of idle,
        // or when we are above our max speed for a very long time (should only happend if entering sleep
        // at high loads)
        if ((cpu_load > max_cpu_load || delta_idle == 0) &&
            !(policy->cur > this_smoothass->max_speed &&
              cputime64_sub(update_time, this_smoothass->freq_change_time) > 100*down_rate_us)) {

                if (policy->cur > this_smoothass->max_speed) {
                        reset_timer(data,this_smoothass);
                }
                ...
                if (cputime64_sub(update_time, this_smoothass->freq_change_time) < up_rate_us)
                        return;
         ...
         */
        if (policy->cur < this_smoothass->max_speed && !timer_pending(&this_smoothass->timer) && nr_running() > 0)
                reset_timer(data,this_smoothass);
         ...
         * minimum sample time.
         */
        if (cputime64_sub(update_time, this_smoothass->freq_change_time) < down_rate_us)
                return;
...
static void cpufreq_idle(void)
{
        ...
        if (!this_smoothass->enable) {
                pm_idle_old();
                return;
        }

        if (policy->cur == this_smoothass->min_speed && timer_pending(&this_smoothass->timer))
                del_timer(&this_smoothass->timer);

        pm_idle_old();

        if (!timer_pending(&this_smoothass->timer))
                reset_timer(smp_processor_id(), this_smoothass);
}
...
static void cpufreq_smoothass_freq_change_time_work(struct work_struct *work)
{
        ...
        for_each_cpu(cpu, tmp_mask) {
                ...
                cpu_load = this_smoothass->cur_cpu_load;
                force_ramp_up = this_smoothass->force_ramp_up && nr_running() > 1;
                ...
                if (force_ramp_up || cpu_load > max_cpu_load) {
                        if (force_ramp_up && up_min_freq) {
                                ...
                                new_freq = this_smoothass->max_speed;
                                relation = CPUFREQ_RELATION_H;
                        }
                }
                ...
                new_freq = validate_freq(this_smoothass,new_freq);

                if (new_freq != policy->cur) {
                        if (debug_mask & SMOOTHASS_DEBUG_JUMPS)
                                printk(KERN_INFO "SmoothassQ: jumping from %d to %d\n",policy->cur,new_freq);

                        __cpufreq_driver_target(policy, new_freq, relation);

                        this_smoothass->freq_change_time_in_idle =
                                get_cpu_idle_time_us(cpu,&this_smoothass->freq_change_time);
                }
                ...
                unsigned int event)
{
        ...
        case CPUFREQ_GOV_LIMITS:
                smoothass_update_min_max(this_smoothass,new_policy,suspended);
                if (this_smoothass->cur_policy->cur != this_smoothass->max_speed) {
                        if (debug_mask & SMOOTHASS_DEBUG_JUMPS)
                                printk(KERN_INFO "SmoothassI: initializing to %d\n",this_smoothass->max_speed);
                        __cpufreq_driver_target(new_policy, this_smoothass->max_speed, CPUFREQ_RELATION_H);
                }
                break;
...
static void smoothass_suspend(int cpu, int suspend)
{
        ...
        smoothass_update_min_max(this_smoothass,policy,suspend);
        if (!suspend) { // resume at max speed:
                new_freq = validate_freq(this_smoothass,sleep_wakeup_freq);

                if (debug_mask & SMOOTHASS_DEBUG_JUMPS)
                        printk(KERN_INFO "SmoothassQ: awaking at %d\n",new_freq);

                __cpufreq_driver_target(policy, new_freq,
                                        CPUFREQ_RELATION_L);

                if (policy->cur < this_smoothass->max_speed && !timer_pending(&this_smoothass->timer))
                        reset_timer(smp_processor_id(),this_smoothass);
        } else {
                ...
                if (debug_mask & SMOOTHASS_DEBUG_JUMPS)
                        printk(KERN_INFO "SmoothassS: suspending at %d\n",policy->cur);
        }
}
...
static int __init cpufreq_smoothass_init(void)
{
        ...
        for_each_possible_cpu(i) {
                ...
                this_smoothass->enable = 0;
                this_smoothass->cur_policy = 0;
                this_smoothass->force_ramp_up = 0;
                this_smoothass->max_speed = DEFAULT_SLEEP_WAKEUP_FREQ;
                this_smoothass->min_speed = DEFAULT_AWAKE_MIN_FREQ;
                this_smoothass->time_in_idle = 0;
                this_smoothass->idle_exit_time = 0;
                this_smoothass->freq_change_time = 0;
                this_smoothass->freq_change_time_in_idle = 0;
                this_smoothass->cur_cpu_load = 0;
                // intialize timer:
...
MODULE_AUTHOR ("Erasmux and LeeDrOiD , modified by ckisgen");
